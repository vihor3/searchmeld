...
管理台“请求日志”默认显示“用户请求”：记录到达后端的搜索、抽取和 MCP 请求的接口、方法、来源 IP、已验证的 Token 身份、HTTP 状态和处理耗时，包括鉴权、参数或限流拒绝。MCP 协议错误和工具错误单独显示，HTTP 200 不等于工具成功。详情可跳转到关联的执行日志，继续查看请求参数、结果和渠道调用；原有历史记录仍在“执行日志”中。

用户入口记录独立于执行用量和计费，沿用“日志保留天数”及“请求日志近窗条数”设置。新记录不复制认证头、Cookie、原始报文或 URL 查询参数；来源 IP 沿用应用的可信代理规则。记录在请求处理结束后尽力写入，数据库故障或进程提前退出可能导致缺失；Nginx 转发前的拒绝、管理接口、预检和健康检查不在此范围内。接口和字段见 [日志文档](docs/admin-api-key.md#511-查看日志和审计日志)。
